...
class MetaModel:
    ...
    # def initialize_prompt_modules(self, mm_use_region_clip=None, mm_use_bbox_fc=False, dtype=torch.float16, device='cpu'): #TODO: add to the training code
...
class ChatUniViMetaForCausalLM(ABC):
        ...
        if self.get_model().use_cluster:
            if input_type == "image": # image
                ...
                token_dict = {'x': image_features,
                              ...
                              'idx_token': torch.arange(image_features.size(1))[None, :].repeat(image_features.size(0), 1),
                              'agg_weight': image_features.new_ones(image_features.size(0), image_features.size(1), 1),
                              'mask': None}

                token_dict = self.get_model().block0(self.get_model().ctm0(token_dict))
                cluster_image_features.append(token_dict["x"])
                # print("ctm0: ", token_dict["x"].shape) # [1, 64, 1024]

                token_dict = self.get_model().block1(self.get_model().ctm1(token_dict))
                cluster_image_features.append(token_dict["x"])
                # print("ctm1: ", token_dict["x"].shape) # [1, 32, 1024

                token_dict = self.get_model().block2(self.get_model().ctm2(token_dict))
                cluster_image_features.append(token_dict["x"])
                # print("ctm2: ", token_dict["x"].shape) # [1, 16, 1024]
                ...
                for id, key in enumerate(events):
                    ...
                    token_dict = {'x': cur_image_features0,
                                  ...
                                  'idx_token': torch.arange(cur_image_features0.size(1))[None, :].repeat(cur_image_features0.size(0), 1),
                                  'agg_weight': cur_image_features0.new_ones(cur_image_features0.size(0), cur_image_features0.size(1), 1),
                                  'mask': None}
                    ...
                    token_dict = {'x': cur_image_features1,
                                  ...
                                  'idx_token': torch.arange(cur_image_features1.size(1))[None, :].repeat(cur_image_features1.size(0), 1),
                                  'agg_weight': cur_image_features1.new_ones(cur_image_features1.size(0), cur_image_features1.size(1), 1),
                                  'mask': None}
                    ...
                    token_dict = {'x': cur_image_features2,
                                  ...
                                  'idx_token': torch.arange(cur_image_features2.size(1))[None, :].repeat(cur_image_features2.size(0), 1),
                                  'agg_weight': cur_image_features2.new_ones(cur_image_features2.size(0), cur_image_features2.size(1), 1),
                                  'mask': None}
            ...
            if input_type == "video":
                image_features = torch.mean(image_features, dim=0, keepdim=False).unsqueeze(0)
                cls_features   = torch.mean(image_features, dim=1, keepdim=False).unsqueeze(0)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        '''
        This function applies CLIP vision encoder and call mm_projector
        ...
        # print('past_key_values:', len(past_key_values) if past_key_values is not None else None)
        # print('labels:', labels.shape if labels is not None else None)
        # print('images:', images.shape if images is not None else None)
        
        # print("self.config.seg_token_idx:", self.config.seg_token_idx)
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, attention_mask, past_key_values, None, labels, seg_token_mask

        # Encode images/videos using CLIP
        if type(images) is list or images.ndim == 5: # video
            concat_images = torch.cat([image for image in images], dim=0) # [batch_size * num_frames, C, H, W]
        ...
        new_input_embeds = [] # list to store modified input_ids per batch
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0: 
                ...
                cur_input_embeds = cur_input_embeds + (0. * self.get_model().mm_projector(vision_tower.dummy_feature)).sum()
                                                    # dummy_feature = torch.zeros(1, self.hidden_size, device=self.device, dtype=self.dtype)
            ...
            if len(image_token_indices) > 1:
                ...
                for i in temp:
                    ...
                    # cur_image_features = []
                    # for _ in i: # for range(len(i))
                    #     cur_image_features.append(image_features[cur_image_idx])
                    #     cur_image_idx += 1
                    cur_image_features = image_features[batch_idx]

                    # if the sublist in the temp list has more than 2 values, it is a video
                    if len(i) > 2:
                        # cur_image_features = torch.stack(cur_image_features, dim=0)
                        cur_image_features = self.project(cur_image_features, input_type="video") # apply projection
                        t, l, n = cur_image_features.size()
                        cur_image_features = cur_image_features.contiguous().view(t * l, n)
                    # if the sublist in the temp list has less than or equal to 2 values, it is an image
                    else:
                        # cur_image_features = torch.stack(cur_image_features, dim=0)
                        cur_image_features = self.project(cur_image_features, input_type="image") # apply projection
                        t, l, n = cur_image_features.size()
                        cur_image_features = cur_image_features.contiguous().view(t * l, n)
                    ...
                    if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config, 'mm_use_im_start_end', False):
                        # Add text before image/video features
                        cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:image_token_start - 1]).detach())
                        cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[image_token_start - 1:image_token_start]))
                        # Add image/video features
                        cur_new_input_embeds.append(cur_image_features)
                        # Add text after image/video features
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer, pretrain_mm_mlp_adapter=''):
        '''
    ...
    # def initialize_prompt_tokenizer(self, tokenizer): #TODO: add this in train code also
