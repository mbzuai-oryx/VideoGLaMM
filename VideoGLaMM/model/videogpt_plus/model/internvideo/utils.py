...
from model.videogpt_plus.model.internvideo.internvideo2 import pretrain_internvideo2_1b_patch14_224
from model.videogpt_plus.model.internvideo.pos_embed import interpolate_pos_embed_internvideo2_new
...
def setup_internvideo2V(config: dict):
    ...
    # print('\033[91mInside setup_internvideo2V\033[0m', 'model.device', model.device, 'config.device', config.device)
...
class VideoTrainProcessor():
    ...
    def preprocess(self, vid_list, use_image=False):
        ...
        Args:
            vid_list: list of numpy arrays , or a list of PIL images
            use_image: bool
        '''
        # if vid_list is a list of PIL images, convert to numpy arrays
        if not isinstance(vid_list[0], np.ndarray):
            vid_list = [np.array(x) for x in vid_list]
